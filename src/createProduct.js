...
function CreateProduct() {
   ...
   function validateForm() {
      ...
      if (fprice == "") {
         missingFlag = true;
      } else if (!/^[+-]?[0-9]{7}\.[0-9]{2}$$/i.test(fprice)) {
         errorFlag = true;
      }
      if (switcher == 'DVD') {
         if (fsize == "") {
            missingFlag = true;
         } else if (!/^[+-]?[0-9]{7}\.[0-9]{2}$$/i.test(fsize)) {
            errorFlag = true;
         }
      } else if (switcher == 'Book') {
         if (fweight == "") {
            missingFlag = true;
         } else if (!/^[+-]?[0-9]{7}\.[0-9]{2}$$/i.test(fweight)) {
            errorFlag = true;
         }
      } else if (switcher == 'Furniture') {
         if (fheight == "") {
            missingFlag = true;
         } else if (!/^[+-]?[0-9]{7}\.[0-9]{2}$$/i.test(fheight)) {
            errorFlag = true;
         }
         if (fwidth == "") {
            missingFlag = true;
         } else if (!/^[+-]?[0-9]{7}\.[0-9]{2}$$/i.test(fwidth)) {
            errorFlag = true;
         }
         if (flength == "") {
            missingFlag = true;
         } else if (!/^[+-]?[0-9]{7}\.[0-9]{2}$$/i.test(flength)) {
            errorFlag = true;
         }
      }
      if (errorFlag) {
